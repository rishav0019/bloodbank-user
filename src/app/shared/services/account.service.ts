...
export class AccountService {
  ...
  constructor(private afs: AngularFirestore) {
    this.accountCollection = this.afs.collection("users");
  }

  getAccounts(): Observable<Account[]> {
    var accountRef = this.afs.collection<Account>("users", (ref) =>
      ref.orderBy("creationDate", "desc")
    );

    return (this.users = accountRef.snapshotChanges().pipe(
      map((actions) =>
        actions.map((a) => {
  ...
  getAccountById(id: string) {
    ...
    this.accountDocument = this.afs.doc(`users/${id}`);
  ...
  setAccount(account: Account) {
    ...
    // console.log("qqqqqqqqqqqqqqqqqqq", id);
  ...
  deleteAccount(account: Account) {
    this.accountDocument = this.afs.doc(`users/${account.id}`);
